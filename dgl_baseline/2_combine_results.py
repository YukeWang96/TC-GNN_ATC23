...
with open('Fig_6a_dgl_gcn.csv', 'w', newline='') as result_file:
    writer = csv.writer(result_file)
    writer.writerow(['Dataset', 'DGL(ms)', 'TC-GNN(ms)', 'Speedup (x)'])
    for i in range(len(col0)):
        writer.writerow([col0[i], col2[i], col1[i], "{:.3f}".format(ratio[i])])

print("\n\n=>Please check [Fig_6a_dgl_gcn.csv] for the results.\n\n")
...
# Write the result, along with the original two columns, to a third CSV file
with open('Fig_6a_dgl_agnn.csv', 'w', newline='') as result_file:
    writer = csv.writer(result_file)
    writer.writerow(['Dataset', 'DGL(ms)', 'TC-GNN(ms)', 'Speedup (x)'])
    for i in range(len(col0)):
        writer.writerow([col0[i], col2[i], col1[i], "{:.3f}".format(ratio[i])])

print("=>Please check [Fig_6a_dgl_agnn.csv] for the results.\n\n")